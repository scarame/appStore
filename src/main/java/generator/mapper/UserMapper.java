...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Select("select id,user_photo,user_name,account,mobile,email,birthday,sex,status,user_type_id" +
            ",update_by,update_date,status from user where account=#{account}")
    ...
    //===更改信息====
    //普通用户
    @Update("update user set user_name=#{user.user_name}," +
    ...
    //管理者
    @Update("update user set user_name=#{user.user_name},password=#{user.password}," +
